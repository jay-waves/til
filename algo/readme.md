...
使用 kernel 风格 [kernel-c-style](../langs/c/kernel-c-style.md), 内容为组合优化.
...
可能会有多个版本的实现:
- 不同编程语言: C, C++, Go. 主语言是 C++, 其他语言版本见 `src/algo/xxx`
- 不同复杂度: 并发版本, 元编程版本
- 不同接口
...
[^1]: 详见 [readme](../langs/cpp/data-types/stl/readme.md)d)

### 数据结构列表

- 栈 (Stack)
- 队列 (Queue), [循环队列](linear/queue.md), [双端队列](linear/deque.md), [优先队列](trees/binary-heap.md)
- 链表 (Linked List). [循环链表](linear/circular-linked-list.md), [双向链表](linear/doubly-linked-list.md), 十字链表, 跳表, 邻接表. 
