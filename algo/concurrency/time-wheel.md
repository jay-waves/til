timer 的两种实现方式：
* 基于排序（最小堆、或红黑树） ：每个 Tick 需要检查堆顶，执行堆顶任务后需要删除并重新平衡。插入和删除任务需要 `O(logN)` 
* 基于时间轮（环形数组）：需要系统定期推进（tick 中断），执行新 slot 内部任务（链表）。插入和删除任务的复杂度均为 `O(1)`，适合任务数量特别大，精度要求不高的场景。

早期 Linux 系统基于时间轮设计 Timer。Linux 4.x 后，利用硬件定时器（精度更高），让每个任务到期被唤醒，而无需手动推进时间轮，此后每个 CPU 只维护一个 rbtree 结构。
