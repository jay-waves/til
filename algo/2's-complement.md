...
讨论负数 $-a$ 的补码表示: **将 b 称为 -a 的补码, a 应是正数原码, 正数补码就是原码.**
...
A: $2^n$ 表示的是形如 $(1000+)_b$ 的二进制数. 比如 $2^3=8=(1000)_b$ (以下也多拿该例说明), $3$ 对应的是 $a,b$ 的位数, 即 $a,b$ 是三位二进制数 ( 比如 `111 + 001`), 那么 a 和 b 互相称为对方相反数的补码.  

注意, $2^n$ 比 $a,b$ 都多一位, 这其实导致了开头式子中 $a,b$ 不能是负的, 否则正数会溢出; 多出的这一位通常在表示中忽略.

## 补码和原码/反码的关系

### Q: 如何理解 a 取反加一 等于补码 -a
