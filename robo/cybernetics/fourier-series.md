...
设角频率为: $$\omega_{0}=\frac{2\pi}{T}$$ ，那么傅里叶级数可改写为： $$f(x)=\sum^{\infty}_{k=-\infty}c_{k}e^{ik\omega_{0} x}$$

频率只出现在 $\omega=k\omega_{0}$ ，复傅里叶系数为： 

$$c_{k}=\frac{1}{T}\int^{T}_{0}f(x)e^{-ik\omega_{0}x}  \, dx $$
