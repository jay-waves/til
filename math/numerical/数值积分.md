...
此时, 原积分可表示为数值形式: $$\int^{b}_{a}f(x)\, dx \approx \int^{b}_{a}\sum^{n}_{i=0}f(x_{i})\cdot L_{i}(x) \, dx =\sum^{n}_{i=0}f(x_{i})\int^{b}_{a}L_{i}(x) \, dx $$
...
**定理: 对任意 $n$, Newton-Cotes 求积公式具有数值稳定性的条件为:** 

$$
\sum_{k=0}^n|\lambda_{k}^{(n)}|\le K 
$$
...
$$R_{1}=-\frac{(b-a)^{3}}{12}f''(\eta),\quad \eta\in(a,b)$$

### Simpson 积分 (n=2)

$$\textstyle\int_{a}^{b}f(x)\,d x\approx{\frac{b-a}{6}}\,[f(x)+4f({\frac{\alpha+b}{2}})+f(b)]$$

$$R_{2}=-\frac{(b-a)^{5}}{2880}{\cal f}^{(4)}(\eta),\eta\in(a,b)$$

### 3/8 Simpson 积分 (n=3)

$$\textstyle\int_{a}^{b}{f(x)\,d x}\approx{\frac{b-a}{8}}[f(a)+3f({\frac{2a+b}{3}})+3f({\frac{a+2b}{3}})+f(b)]$$

$$R_{3}=-\frac{(b-a)^{5}}{6480}{f}^{(4)}(\eta),\eta\in(a,b)$$

### 复化梯形积分

将区间分为多个小区间, 每个小区间独立使用 New-Cotes 求积. **这样能保证结果数值收敛.**
...
以*梯形复化求积公式*为例, 假设 $f(x)$ 在区间上二阶连续可导:

$$
\int_a^b f(x)dx \approx \frac{h}{2}[f(a)+f(b)+2\sum_{k=1}^{n-1}f(a+kh)]
$$

复化梯形积分满足 $\sum^{n}|\lambda_{k}^{(n)}|=b-a$, 具有数值稳定性.

## Gauss 型积分公式

定义高斯求积: $$Q(f)=\sum^{n}_{i=0}A_{i}f(x_{i}),\quad A_{i}=\int^{b}_{a}\rho(x)L_{i}(x) \, dx $$ 其中 $x_{i}$ 是 $n$ 次正交多项式 $\varphi(x)$ 的互异实根. $L_{i}$ 是以 $x_{i}$ 为节点的拉格朗日基函数.

注意, $\varphi(x)$ 是定义在关于权函数 $\rho(x)$ 的[正交多项式系](../calculus/正交多项式.md)上的: $$\int^{b}_{a}\rho\ \varphi_{m}\varphi_{n}  \, dx =0,\quad m\neq n$$
