...
多项式插值 (Polynomial interpolation) 是寻找通过点 $\set{(x_{0},y_{0}),\ (x_{1},y_{1}),\ (x_{2},y_{2}),\ \dots,\ (x_{n}, y_{n})}$ 的最低次多项式曲线: $$P_{n}(x)=a_{0}+a_{1}x+a_{2}x^{2}+\cdots+a_{n}x^{n}$$ 使得 $P(x_{i})=y_{i}$. 对于 $n$ 次多项式 $P_{n}(x)$, 根据[线性方程组](../linalg/线性方程组/线性方程组的解.md)理论, 至少需要 $n+1$ 个点 $(x_{i},y_{i})$ 来确定**唯一**解: 

$$\array{P_{n}(x_{0})=y_0\\P_{n}(x_{1})=y_{1}\\ \vdots\\ P_{n}(x_{n})=y_{n} }$$

可以通过矩阵直接求解系数 ([范德蒙方法](../algebra/环/多项式环.md)), 也可以通过拉格朗日插值法和牛顿插值法.

**插值是函数的相反操作**, 函数通过曲线确定点, 插值通过点寻找曲线. **插值是级数拟合的一种方式**, 其他方式还有: [fourier-series](../../robotics/cybernetics/fourier-series.md), [泰勒级数](../calculus/泰勒级数.md), 幂级数和函数, 线性回归.
...
![](http://oss.jay-waves.cn/til/差商表.avif)
...
给定 $n+1$ 个点 $x_0 \cdots x_i$, 并给定其中 $m+1$ 个点的导数 (总的坐标集合仍为 ${} n+1 {}$), 构建最多 $m+n+1$ 阶多项式:
...
构造时, 先利用 $x_{0},x_{1}\dots,x_{n}$ 求 $P_{n}(x)$, 然后构造: $$H(x)=P_{n}(x)+Q_{m}(x)\omega_{n+1}(x)$$
