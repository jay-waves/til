...
- 压缩性: 当 $ x in(a , b) , quad | phi(x)' | lt.eq L < 1 $, $L$ 为常数

满足上述条件可以保证求解的序列线性收敛, 并可以成立误差估计式:


$ | s - x_k | lt.eq frac(L^k, 1 - L) | x_1 - x_0 | $ $ | s - x_k | lt.eq frac(L, 1 - L) | x_k - x_(k - 1) | $
...
求解到某阶 $k gt.eq K$ 时进行判断:

- $r = 1$, 线性收敛, $0 < c lt.eq 1$
- $r = 2$, 平方收敛
- $r > 1$, 统称为超线性收敛

在先前的收敛性判断中, 简单迭代法是线性收敛的, 若能够满足以下条件:

- $phi^(( i )) ( s ) = 0 , ( i = 1 , 2 , dots.c , m - 1 )$
- $phi^(( m )) ( s ) eq.not 0$
...
$ x_(k + 1) - s = phi'(epsilon_k) ( x_k - s ) , quad x_(k + 2) - s = phi' ( epsilon_(k + 1) ) ( x_(k + 1) - s ) $

对上述式子进行变型：

$ frac(x_(k + 1) - s, x_k - s) approx frac(x_(k + 2) - s, x_(k + 1) - s) arrow.r s approx x_k - frac(( x_(k + 1) - x_k )^2, x_(k + 2) - 2 x_(k + 1) + x_k) $
...
- $f''(x)$ 连续且 $f'(x) eq.not 0$, 那么 $x_0$ 在靠近 $s$ 的某个范围内收敛于 $s$.
- $f''(x) eq.not 0$, 且 $x_0 eq.not s$, 则迭代法是平方收敛的.
...
如果我们固定初始点 $( x_0 , f(x_0) )$, 那么我们可以得到#strong[单点割线法] (一阶收敛):
