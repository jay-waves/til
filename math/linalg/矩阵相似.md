...
矩阵相似描述了同一线性变换在不同基底下的表示, $P$ 是某种[过渡矩阵](向量分析/空间基底变换.md), $P\mathbf{x}$ 将坐标 $\mathbf{x}$ 转换为新基底下的坐标, $AP\mathbf{x}$ 指对新基底下的向量进行 $A$ [线性变换](向量分析/线性变换.md), 然后 $P^{-1}AP\mathbf{x}$ 将坐标重新转换回原基底. 一系列操作的效果, 等价于直接进行 $B\mathbf{x}$ 线性变换.
...
注: 在 $A$ 的特征方向上, 向量只有长度变化, 而[向量在不同基底表示下是同一的](向量分析/空间基底变换.md). 将特征向量的坐标 $\xi$ 从旧基底变换到新基底下 $P^{-1}\xi$, 向量模数仍变化 $\lambda$ 倍.
