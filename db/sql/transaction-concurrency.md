**并发控制保证[事务](data-security.md)的隔离性和一致性**. 数据库中, 并发控制通过三种手段实现: **锁, 时间戳和快照隔离**.
...
数据库**事务**的隔离级别定义:
1. 可串行化: 理想情况, 所有事务的并发操作等价于[串行操作](../../security/software-analysis/并发安全/happens%20before.md).
...
读写锁详见 [thread-sync](../../os/proc/thread-sync.md)
...
循环等待资源, 造成死锁. 详见 [thread-sync](../../os/proc/thread-sync.md)
