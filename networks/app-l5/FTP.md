...
数据传输前, 建立两个 [TCP](../transport-l4/TCP.md) 连接, FTP 服务器的端口分别为 20 和 21. 21 端口是命令传输端口, 20 端口为数据传输端口.
...
主动模式下, FTP 服务器的[Firewall](../firewalls&vpn/Firewall.md)只需要开启 21 端口的准入, 而客户端需要开启 20 端口的准入.
...
客户端的[Firewall](../firewalls&vpn/Firewall.md)通常"准出", 因此不用多余配置.
