...
现代协议 [(如 WiFi, 4G)](wireless/无线通信技术.md) 是先模数转换, 然后调制解调数字信号, 称为*数字调制*. 传统信号 (如 FM, AM) 则*模拟调制*, 然后再转化为数字信号.

## 数据传输方式

比如 [RS-232](../arch/embedded-system/外设与总线.md) 总线就是**串行-异步-半双工-点对点**通信; 以太网 (Ethernet) 是**光纤串行-同步时序-全双工-分组交换**通信.
