...
- [公私钥体系](../../public-key-crypto/digital-signature/digital-signature.md)有**不可否认性**; 而 MAC 双方都有密钥, 都可以伪造消息. 
- MAC 计算速度更快, 但不适合大规模分发.

### MAC 实现方式

- 基于[msg-digest](../msg-digest.md), 比如 [HMAC](HMAC.md) / [KMAC](KMAC.md) / [UMAC](UMAC.md)
...
2. $C_{k}(M)$ 在值域中分布均匀, 阻止 *基于选择明文的穷举攻击*. 分布均匀, 指随机选消息 $(M, M')$, $Pr[C_{K}(M)=C_{K}(M')]=2^{-n}$. 这样保证, 敌手可以选择明文攻击时, [需平均穷举 $2^{n-1}$次](../birthday-attack.md)
