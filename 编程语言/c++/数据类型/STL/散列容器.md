无序容器 (散列容器, unordered containers) 基于[哈希表](../../../../算法/哈希表/hash%20table.md)实现, 不进行自动排序. 查找速度最快, 复杂度为 $O(1)$.
...
`std::unordered_map` 的桶是链表, 链式处理哈希冲突; flat_hash 的桶则是平坦数组, 探测法处理哈希冲突. flat_hash 装载比率较高时 (负载因子超过 0.7), 会动态增长数组 (rehash), 短时间要重新计算哈希并复制数据, 有性能抖动, 详见 [Algorithm/哈希表](../../../../算法/哈希表/hash%20table.md).
