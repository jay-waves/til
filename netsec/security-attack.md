...
服务器为减小密码泄露的危害性, 常采用[密码散列](cryptography/msg-digests/消息摘要.md)形式存储密码, 而不使用原始明文值. 为防止彩虹表攻击, 服务器存储密钥时还会加入随机盐值 (salting) 来混淆密码明文和哈希之间的对应关系: $\mathrm{Hash}(salt\ \Vert\ password)$. 注意, 加盐没有额外客户端步骤, 且对暴力破解攻击没有额外抗性.
...
Collision Attack. 参考 [哈希算法-生日攻击](cryptography/msg-digests/生日攻击.md).
...
[长度扩展攻击](cryptography/msg-digests/MD%20结构/长度扩展攻击.md)
...
*PAC (Pointer Authentication Codes, 指针认证码)*. 在地址上附加短 [MAC](cryptography/msg-digests/消息认证码/MAC.md), 在地址进行 加载/分支 操作前, 检查认证码. 需要硬件支持, 并且认证码密钥直接由硬件持有. 
