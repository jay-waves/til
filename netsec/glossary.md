...
[Access Control List, 访问控制列表](../networks/防火墙/Firewall.md), 用于定义哪些用户或系统进程可以访问系统资源以及权限级别.  
...
高效软件模糊测试框架, 见 [模糊测试技术](软件安全/模糊测试.md)
...
Intrusion Detection System, [入侵检测系统](../networks/防火墙/IDPS.md), 用于检测网络或系统中的恶意活动.  

### IPS

Intrusion Prevention System, [入侵防御系统](../networks/防火墙/IDPS.md), 与 IDS 类似, 但同时具有阻止威胁的能力. 
...
[Network Address Translation, 网络地址转换](../networks/防火墙/NAT.md), 用于在局域网和公网之间转换 IP 地址. 
...
[PGP](../networks/应用层/PGP.md), Pretty Good Privacy, 用于加密和签名的公钥加密程序. 
...
先入后出的栈数据结构, 见 [binary heap](Algorithm/内核/list.md). 也指进程内存空间的一种结构, 见 [linux 内存空间分布](../os/内存管理/linux%20内存空间分布.md).
...
计算机网络传输层的[传输控制协议 (TCP)](../networks/传输层/TCP.md), 提供可靠点对点连接.
