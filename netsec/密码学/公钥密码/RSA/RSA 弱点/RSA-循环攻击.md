...
... 由[欧拉函数](../../../../../math/numth/欧拉函数.md)知, $\phi(N)=\prod^{s}_{i=1}\left( p_{i}^{k}-p_{i}^{k-1} \right)$, 其中 $p_{i}$ 是 $N$ 的素因子. $\phi(p)=p-1$ 有素因子 $r$, 那么 $\phi(p-1)$ 中就有因子 $r-1$. 出于类似的原因, 也让 $r-1$ 存在较大因子.
