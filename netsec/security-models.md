...
- [Bell-Lapadula 机密性模型, Biba 完整性模型.](security-models.md) 基于强制访问控制 (MAC)
- [OSI 模型和 TCP/IP 协议栈安全模型](../networks/internet.md), 主要关注网络层, 传输层和应用层安全技术.
- 各类[防火墙](../networks/firewalls&vpn/Firewall.md)
- [非军事区模型](../networks/net-devices&topology.md) (DMZ, Demilitarized Zone), 用于隔离企业网络中的互联网和内部网络之间的访问.
...
浏览器使用访问控制列表来管理文件权限, 数据库使用 `GRANT, REVOKE` 等 [SQL](../db/SQL/ReadMe.md) 语句来定义权限. 该过程也被称为 "授权".

### RBAC

基于角色的访问控制, Role-Based Access Control, RBAC. 将权限附于角色中, 再将用户与特定角色绑定.

![](http://oss.jay-waves.cn/til/RBAC.avif)
