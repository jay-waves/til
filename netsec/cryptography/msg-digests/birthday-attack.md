...
攻击[密码学杂凑函数](msg-digest.md)的目标是: 伪造消息, 使其与原消息杂凑值相同
...
对于[带消息附录的签名形式](../public-key-crypto/digital-signature/digital-signature.md), 签名为 ${} \set{\mathrm{hash}(M)}_{sk}\Vert M {}$. 如果能伪造一个 $M'$, 使 $\mathrm{hash}(M)=\mathrm{hash}(M')$, 就能伪造出新签名 $\set{\mathrm{hash}(M')}_{sk}\Vert M'$, 但名义上签发者 ($sk$ 拥有者) 并不知情.
