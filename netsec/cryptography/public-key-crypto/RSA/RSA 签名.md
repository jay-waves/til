...
明密文长度太长, 为加快速度减少成本, 通常仅对明文的[杂凑值](../../msg-digests/msg-digest.md)进行签名. 该签名方法的输出长度易控制, 并且由于杂凑函数带来的抗碰撞性, 可以有效抵御 [RSA 签名篡改攻击](RSA%20弱点/RSA%20CCA%20Attack.md).
...
一般情况下, 带附录的签名长度更长. 对于 RSA 签名, 若消息长度小于 $\frac{len(n)}{2}$ `bits`, 使用[带消息恢复的 RSA 签名](../digital-signature/digital-signature.md); 否则使用[带附录的 RSA 签名](../digital-signature/digital-signature.md).
...
[^1]: 注意 MAC 的规范正相反, 要求先验证完整性再解密, 加快验证速度, 见 [安全协议/混合加密](../../security-protocols/混合加密.md)