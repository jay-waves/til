...
*VMA, Virtual Memory Area, 虚拟内存区域*, 存储虚拟内存空间和磁盘的映射关系. 进程内存分布详见 [Linux 内存空间分布](../../System/Memory/Linux%20内存空间分布.md), 进程大致有如下几种 VMA:
- 代码 VMA, 只读, 可执行, 有映像文件
...
当段 (section) 数量增多时, 由于虚拟内存映射以页为单位, 每个段都会产生多余空间浪费. 
Linux 的解决方案是将同权限 (读, 写, 执行) 的段合并为一个段 (**segment**), 一个 Segment
对应一个一个 VMA, 从而减少页内碎片. 描述 Segment 的结构叫程序头 (Program Header)
