...
可以发现矩阵 $A=[\mathbf{\alpha}, \mathbf{\beta}]$ 将基底变换为 $\alpha=(a, c)$ 和 $\beta=(b,d)$, 张成 (线性组合成) 子空间记为 $\mathbb{C}(A)$. 注意, $x,\ y$ 仍是新基底下的坐标, 而新基底 $\alpha=(a,c)$ 和 $\beta=(b,d)$ 是用单位基底表示的. 除了[空间基底变换](空间基底变换.md)过程, 线性变换的另一个形象体现是[二次型形状变换](矩阵相合.md), 本质也是基底变换.
