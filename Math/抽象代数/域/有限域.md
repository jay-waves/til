...
对于有限域 ${} GF(p^{n})\simeq GF(p)[x]/(f(x)),\quad def(f)=n {}$, $\alpha$ 是不可约多项式 $f(x)$ 在更大扩域 ($GF(2^{m}),\quad m>n$) 上的根. 极小多项式是以 $\alpha$ 为根的度最小首一多项式 $p(x)\in GF(p^{n}) {}$, 其系数 $c_{i}\in GF(p)$, 次数可能小于 $f(x)$. 假设在扩域上, $f(x)=0$ 有解 $\alpha,\beta\in GF(p^{m}),\quad \alpha, \beta\not\in GF(2^{n})$, 此时在扩域上 $f(x)$ 可分解为 $f(x)=(x-\alpha)(x-\beta)P(x)$, 显然并不是 $\alpha$ 的极小多项式, 包含了多余的项 ${} (x-\beta) {}$.
...
设 $\alpha$ 在扩域上的极小多项式为 $m(x)=c_{k}x^{k}+c_{k-1}x^{k-1}+\dots$, 根据有限域的 Frobenius 自同构: $\alpha:\to \alpha^{p}$, 可以得到: $$\begin{align}
m(\alpha^{p}) & =c_{k}(\alpha^{p})^{k}+c_{k-1}(\alpha^{p})^{k-1}+\dots+c_{1}\alpha^{p}+c_{0},\quad c_{i}\in GF(p) \\
 & =(c_{k}\alpha^{k})^{p}+(c_{k-1}\alpha^{k-1})^{p}+\dots.+(c_{1}\alpha)^{p}+c_{0} \\
 & =c_{k}\alpha^{k}+c_{k-1}\alpha^{k-1} +\dots+c_{1}\alpha+c_{0} \\
 & =m(\alpha) =0
\end{align}$$

同理可以扩展到 $\alpha^{p^{i}}$ 上. 因此在扩域上, $\alpha$ 的共轭元素是其极小多项式的所有根, 即可以分解为: 

$$m(x)=(x-\alpha)(x-\alpha^{p})(x-\alpha^{p^{2}})\dots(x-\alpha^{p^{k-1}})$$, 其中 $k$ 是元素的扩展次数.
