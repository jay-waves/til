`kfifo` is the linux kernel's implementation of queue, first data in is the first data out. kfifo is a **circular queue** defined in `include/linux/kfifo.h` and `lib/kfifo.c`.
