...
**攻击类型:**
- Syn Flood: 发送大量 [TCP SYN](../网络通信/传输层/TCP.md) 报文, 使服务器建立大量半连接状态, 耗尽服务资源. 防御技术为 SYN Cookie, 收到 SYN 报文后, 不建立连接, 而是将头部内容的散列值当作序列号放入 SYN ACK 报文中返回, 之后收到含正确响应编号的 ACK 报文后, 才正式建立连接.
- [ICMP](../网络通信/网络层/ICMP.md) Flood: 也叫 Ping Flood, 发送大量 ICMP echo request.
- UDP/IP Flood: 发送大量碎片 [UDP](../网络通信/传输层/UDP.md) 报文, 占用内存.
...
[ARP](../网络通信/网络层/ARP.md) 协议会信任所有收到的 ARP 响应, 如果更新了错误的 IP-MAC 映射, 会将 IP 包转发到错误的 (恶意的) MAC 地址设备.

### 1.3.2 DNS 欺骗

攻击者给请求者发送错误的响应, 引导用户至恶意网站. 也称为 **DNS 缓存中毒攻击**, **DNS 污染**.

针对 [DNS](../网络通信/应用层/DNS.md) 不同阶段, 有两种表现形式:
...
攻击者伪造 [ICMP](../网络通信/网络层/ICMP.md) 重定向消息, 使主机将流量发往攻击者伪造的网关 (某个攻击者控制的 IP), 从而建立恶意路由.
