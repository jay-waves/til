...
- [OSI 模型和 TCP/IP 协议栈安全模型](../网络通信/网络体系结构.md), 主要关注网络层, 传输层和应用层安全技术.
- 各类[防火墙](../网络通信/防火墙/Firewall.md)
- [非军事区模型](../网络通信/网络设备及拓扑.md) (DMZ, Demilitarized Zone), 用于隔离企业网络中的互联网和内部网络之间的访问.
...
浏览器使用访问控制列表来管理文件权限, 数据库使用 `GRANT, REVOKE` 等 [SQL](../数据库/SQL/ReadMe.md) 语句来定义权限. 该过程也被称为 "授权".
