...
Linux 的内存策略是：尽可能地利用空闲内存做缓存，不追求空闲内存大小。
* swap used：已经被换出到 swap 分区的内存大小。频繁 swap in/out 可能表示内存紧张
* buff：用于块设备元信息的缓冲区。历史上用于 inode、superblock 等结构
* cache：指**文件数据的页缓存**。
* free：当前完全空闲、未被使用的物理页。但 free 小不代表内存不足。

free 小不代表 linux 内部不足，内存中存在大量 页缓存、缓冲区缓存、Slab 缓存。
只有 Swap 颠簸，Cache 大量被回收时，存在内存压力。
...
声明虚拟内存时, 不会实际映射到物理内存. 仅当使用虚拟内存时, 通过触发*[缺页中断 (Page Fault)](virtual-mem.md)*, 才会完成映射. 
