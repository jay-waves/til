...
| `nslookup`      | 查询 [DNS](../../networks/应用层/DNS.md), 简单信息(IP, CNAME)    | `nslookup google.com`        |                 |
| `dig`           | 查询 [DNS](../../networks/应用层/DNS.md), 详细信息(A, MX, TXT)   |                              |                 |
...
4. (L3) 检查 `ip addr` 
5. 检查路由是否正确 `ip route get <IP>`，如是否走向预期网关
...
iftop 侧重于持续监控网络连接, netstat 则用于一次性检查网络配置和状态. **通常而言, 用 iftop/nethogs 查看哪个端口在占用带宽, 然后用 netstat/lsof 查看该端口对应的进程.**
