...
对于目录而言, `x` 表示"可以进入目录, 但不能查看目录内容". `w` 表示"有编辑权限", 先有目录的 `w` 权限, 才能修改文件. `rwx` 也分别对应数字 `421`, 权限可简写为三者之和. 如 `rw-` 对应 `3`. 一个文件能否被删除, 主要看目录是否有其所在目录的写权限.

用 `chmod` 修改文件权限, 推荐用数字模式 (1, 2, 4). `chown` 修改文件属主. 用 `umask` 修改文件默认权限, 推荐使用字符缩写 (u, g, o, a).
