...
| smb                                  |           |
| sysv        |             |
| exFAT      | Extedned File Allocation Table, 微软开发, 作为 FAT32 改进版本. 跨平台兼容性较好, 用于 U 盘或外部硬盘. 文件最大 16EB |
| jffs2      | 日志型文件系统, 崩溃一致性好. 多用于 NOR Flash.    |
| yaffs2      | 类似 jffs2. 用于 NAND Flash, 利用了一些特性.              |
| ubifs    | 用于大容量 NAND Flash              |
| squashfs      | 压缩文件系统, 但是只读.            |
| fuse | fielsystem in userspace, 通过访问 fuse daemon，将任意源挂载为文件系统 |
...
[^1]:  见 linux bitmap 页缓存. 当不指定 `O_DIRECT` 标志时, 默认非直接 I/O, 读写操作和内核缓存交互, 由内核决定何时与磁盘交互.
