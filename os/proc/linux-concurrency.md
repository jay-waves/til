...
2. 抢占调度
3. 中断
4. [内存模型的一致性问题](../../hw/computer/内存一致性.md)
5. 对象生命周期: 退出及释放安全
6. 死锁 (阻塞等待问题)

内核提供的互斥保护机制包括: *中断屏蔽, 原子操作, 自旋锁, 信号量, 互斥体*. 可参考 [thread-sync](thread-sync.md), 以及[硬件内存模型](../../hw/computer/内存一致性.md)
...
假设 CPU 单核心持有自旋锁进入临界区, 此时刻被硬件中断打断, 同时中断处理程序也试图获取同一个自旋锁. 此时会发生: 中断程序发现锁被占用, 自选等待; 实际持有锁的代码已因中断暂停, 释放了 CPU 使用权, 因此无法释放锁, 这导致了[死锁](thread-sync.md).
...
[信号量 (Semaphore)](thread-sync.md) 是经典的同步与互斥工具. 可以保护临界区, 当获取不到信号量时, 进程进入休眠等待, 而不是忙等待 (自旋锁). 在内核中, 信号量多用于同步, 内核更推荐直接用 mutex 作为临界区互斥手段.
