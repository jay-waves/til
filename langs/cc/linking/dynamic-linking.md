...
以 Linux 为例, 首先[通过 `execve()` 系统调用装载可执行文件](executable-loading.md). 对于静态链接程序, 接下来控制权移交给程序入口 (ELF 头中 `e_entry` 指定的入口); 对于动态链接程序, 此时共享库还未装载, 内核首先根据 ELF `.interp` 段装载动态链接器, 然后将控制权移交给动态链接器.
