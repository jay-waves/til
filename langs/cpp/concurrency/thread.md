...
要在多个线程间安全共享数据, 避免[数据竞争](../../../sec/software-analysis/并发安全/数据竞争.md), 需要使用各类[同步原语](../../../os/proc/thread-sync.md). 比如, 通过 `lock()` 获取一个*互斥量 (Mutex)*. 与传参方式相比, 共享数据不一定效率更高, 因为锁和原子操作是比较耗时的.
