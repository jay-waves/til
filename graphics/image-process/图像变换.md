卷积和相关知识见[数字信号处理:卷积与相关](../../math/sigs-sys/convolution.md), 图像中卷积一般指互相关 (cross correlation): $$\large O[i,j]=(I*K)[i,j]=\sum\limits_{m}\sum\limits_{n}I[i+m,j+n]\cdot K[m,n]$$ 其中 $K$ 称为变换核.
...
详见 [fourier-transform](../../math/sigs-sys/fourier-transform.md), 将图像变为频域图像, 减小计算量.
