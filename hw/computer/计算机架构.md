...
由于 CPU 核心频率提高, 内存 (DRAM) 频率跟不上, 于是产生了系统总线 (system bus), CPU 使用倍频方式与内存通信. 同时诞生了[多级缓存技术](cache.md), **缓存** (SRAM) 速度比内存更高, 作为 CPU 数据的缓冲.
...
<img src="../../assets/hw/computer-arch.avif" alt="早期计算机设备架构" width="700">
...
- [存储设备](storage.md): 机械硬盘, HDD --> 固态硬盘, SSD
...
<img src="../../assets/hw/hw-modern-computer-integrated-architecture.avif" alt="更现代的架构" width="700">

[^1]: 程序员的自我修养--连接, 装载与库. 俞甲子等. P7

### 嵌入式控制器

EC (Embedded Controll), 在手机上称为 PMIC (功率管理集成电路), 在台式机称为 SIO.

主要负责: 电源和电池管理, 温度检测和风扇控制, 故障检测.

### BIOS 芯片

负责存储 [BIOS](../../os/linux/linux-boostrap.md). 过去认为该芯片是 ROM, 但目前 (2025) 多用 [NOR Flash](storage.md), 因此现在的 BIOS 程序可以被升级更新.
...
[^2]: 无线通信, 调制解调内容, 详见 [wireless-comm](../../net/wireless/wireless-comm.md). 数字滤波内容, 详见 [信号处理](../../robotics/cybernetics/signals.md).

<img src="../../assets/hw/hw-mobile-hardware-architecture.avif" alt="" width="500">
